...
def run_get_response_log_probs(
    ...
    final_dict ={}
    final_dict['log_probs'] = log_probs_y
    if return_token_entropy: 
        final_dict['token_entropy'] = run_compute_entropy(logits)
    return final_dict
...
def run_compute_grpo_clip_loss(
    ...
    p_at = ratio * advantages
    clipped = torch.clip(ratio, 1-cliprange, 1+cliprange)
    clipped_at = clipped * advantages
    negative_min = -torch.minimum(p_at, clipped_at)
    return negative_min, {}
...
def run_grpo_microbatch_train_step(
    ...
    """
    loss,meta_data = run_compute_policy_gradient_loss(policy_log_probs, loss_type, raw_rewards,
